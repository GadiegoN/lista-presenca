...
export function AttendanceTable({
  ...
  return (
    <div className="overflow-x-auto overflow-y-hidden relative pb-2 sm:rounded-b-xl">
      <table className="border-collapse w-max min-w-full text-[13px] sm:text-sm">
        <thead>
          <tr className="bg-gray-100 text-gray-700">
            <th className="px-2 py-2 border text-left sticky left-0 bg-gray-100 z-10">
              Jogador
            </th>
        ...
        <tbody>
          {players.map((p: any) => (
            <tr key={p.id} className="hover:bg-gray-50">
              <td className="border px-2 py-1 font-medium text-gray-700 sticky left-0 bg-white z-10 whitespace-nowrap">
                {p.name}
              </td>
              ...
              {weekDays.flatMap((day: Date) =>
                events.map((ev: any) => {
                  ...
                  return (
                    <td
                      key={`${p.id}-${ev.id}-${date}`}
                      className="border text-center py-1 min-w-[60px] sm:min-w-[75px]"
                    >
